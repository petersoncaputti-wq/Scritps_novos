<#
.SYNOPSIS
    Cria usuários do ProjectWise em lote a partir de arquivos CSV ou Excel.
...
.DESCRIPTION
    ...
    O usuário será criado sempre como Bentley IMS.
    O nome de usuário Bentley IMS será preenchido com o mesmo valor do e-mail.
...
function Require-Command {
    ...
    foreach ($nome in $Nomes) {
        if (-not (Get-Command $nome -ErrorAction SilentlyContinue)) {
            throw "Cmdlet obrigatório não encontrado: $nome. Verifique se o módulo PWPS_DAB está instalado e carregado."
        }
    }
}
...
function Selecionar-ArquivoEntrada {
    ...
    $dialog.Title = "Selecione a planilha de usuários do ProjectWise"
...
function Selecionar-PastaExtracaoUsuariosPorDescricao {
    ...
    try {
        ...
        $dialog.Description = "Selecione onde salvar os arquivos Excel por descrição"
        ...
        Write-Log "Falha ao abrir o seletor de pasta para extração: $($_.Exception.Message)" "WARN"
    }

    Write-Status "Nenhuma pasta foi selecionada para a extração. Será utilizada a pasta padrão: $PastaPadrao" "WARN"
    return $PastaPadrao
}
...
function Importar-DadosUsuarios {
    ...
    elseif ($ext -eq ".csv") {
        ...
        throw "Extensão não suportada: $ext. Use .csv, .xlsx ou .xlsm."
    }
...
function Validar-RegistroUsuario {
    ...
    if ([string]::IsNullOrWhiteSpace($Registro.UserName)) {
        $erros.Add("Nome de usuário vazio")
    }
...
function Conectar-ProjectWise {
    ...
    if ($SomenteValidar) {
        Write-Status "Modo 'Somente validar': a autenticação no ProjectWise não será realizada."
        return $null
    }

    Write-Status "Abrindo a autenticação do ProjectWise..."
    ...
    Write-Status "Autenticação realizada. Usuário atual: $nomeAtual" "OK"
...
function Set-IdentidadeBentleyIMS {
    ...
    if ([string]::IsNullOrWhiteSpace($Email)) {
        throw "Não foi possível definir o nome de usuário Bentley IMS porque o e-mail está vazio."
    }
...
function Add-UsuarioEmAcessos {
    ...
    if ($NaoAdicionarAcessos) {
        return "Adição a grupos e listas ignorada conforme a opção selecionada"
    }
    ...
    foreach ($grupo in $Registro.Groups) {
        try {
            ...
            if (Test-UsuarioEmColecao -Usuario $Usuario -Membros $membros) {
                $resultados.Add("Grupo '$grupo': usuário já adicionado")
                continue
            }

            Add-PWUserToGroup -InputUser @($Usuario) -GroupName $grupo -ErrorAction Stop | Out-Null
            $resultados.Add("Grupo '$grupo': usuário adicionado")
        }
        catch {
    ...
    foreach ($userList in $Registro.UserLists) {
        try {
            ...
            if (Test-UsuarioEmColecao -Usuario $Usuario -Membros $membros) {
                $resultados.Add("Lista de usuários '$userList': usuário já adicionado")
                continue
            }

            Add-PWUserToUserList -InputUser @($Usuario) -UserList $userList -ErrorAction Stop | Out-Null
            $resultados.Add("Lista de usuários '$userList': usuário adicionado")
        }
        catch {
            $resultados.Add("Lista de usuários '$userList': erro - $($_.Exception.Message)")
        }
    }

    if ($resultados.Count -eq 0) {
        return "Nenhum grupo ou lista de usuários informado"
    }
...
function Processar-Registros {
    ...
    foreach ($registro in $Registros) {
        try {
            ...
            if ($erros.Count -gt 0) {
                $mensagem = $erros -join "; "
                Write-Status "Linha $($registro.Linha): inválida - $mensagem" "WARN"
                $resultados.Add((New-Resultado -Registro $registro -Status "INVALIDO" -Acao "Validacao" -Mensagem $mensagem))
                continue
            }

            if ($SomenteValidar) {
                Write-Status "Linha $($registro.Linha): válida - $($registro.UserName)" "OK"
                $resultados.Add((New-Resultado -Registro $registro -Status "VALIDO" -Acao "SomenteValidar" -Mensagem "Registro validado. Nenhuma alteração foi realizada."))
                continue
            }
            ...
            if ($verificacaoUsuario.Existe) {
                if ($AtualizarExistentes) {
                    ...
                    $usuario = Buscar-UsuarioPW -Registro $registro
                    $acao = "Atualizado"
                    Write-Status "Linha $($registro.Linha): usuário existente atualizado - $($registro.UserName) (localizado por $($verificacaoUsuario.EncontradoPor))" "OK"
                }
                else {
                    $acao = "JaExistia"
                    Write-Status "Linha $($registro.Linha): usuário já existente - $($registro.UserName) (localizado por $($verificacaoUsuario.EncontradoPor))" "WARN"
                }
            }
            else {
                $usuario = Criar-UsuarioPW -Registro $registro
                if (-not $usuario) {
                    throw "O cmdlet de criação foi executado, mas o usuário não foi localizado em seguida."
                }

                $acao = "Criado"
                Write-Status "Linha $($registro.Linha): usuário criado - $($registro.UserName)" "OK"
            }

            $statusAcessos = Add-UsuarioEmAcessos -Usuario $usuario -Registro $registro
            $resultados.Add((New-Resultado -Registro $registro -Status "OK" -Acao $acao -Mensagem "Registro processado com sucesso." -Acessos $statusAcessos))
        }
        catch {
...
function Exportar-UsuariosPorDescricoes {
    ...
    if ($descricoes.Count -eq 0) {
        Write-Status "Nenhuma descrição foi informada na planilha. A exportação por descrição foi ignorada." "WARN"
        return @()
    }
    ...
    foreach ($descricao in $descricoes) {
        Write-Status "Gerando arquivo da lista processada para a descrição: $descricao"
...
function Mostrar-ResumoFinal {
    ...
    Write-Status "Usuários criados            : $criados" "OK"
    Write-Status "Usuários atualizados        : $atualizados" "OK"
    Write-Status "Usuários já existentes      : $jaExistiam" "WARN"
    Write-Status "Linhas apenas validadas     : $validos"
    Write-Status "Linhas inválidas            : $invalidos" $(if ($invalidos -gt 0) { "WARN" } else { "OK" })
    Write-Status "Erros de processamento      : $erros" $(if ($erros -gt 0) { "ERROR" } else { "OK" })
    Write-Status "Relatório completo          : $ArquivoRelatorioFinal" "OK"
    if (@($ArquivosUsuariosPorDescricao).Count -gt 0) {
        Write-Status "Arquivos Excel por descrição:" "OK"
    ...
    if ($itensComProblema.Count -gt 0) {
        Write-Status ""
        Write-Status "Linhas que precisam de atenção:" "WARN"
    ...
    if ($itensExistentes.Count -gt 0) {
        Write-Status ""
        Write-Status "Usuários que já existiam e não foram recriados:" "WARN"
...
try {
    Write-Status "ProjectWise - criação de usuários em lote"
    if ($ModoIntegrado) {
        Write-Status "Execução iniciada pela Central de Scripts ProjectWise."
        $NaoPausar = $true
        if (-not $GerarModelo -and [string]::IsNullOrWhiteSpace($CaminhoArquivo)) {
            throw "O caminho da planilha deve ser informado no modo integrado."
        }
    }
    ...
    if ($linhas.Count -eq 0) {
        throw "A planilha não possui registros."
    }
    ...
    try {
        ...
        if ($login -and -not $SomenteValidar) {
            try {
                ...
                Write-Status "Sessão do ProjectWise encerrada."
            }
            catch {
                Write-Log "Falha ao encerrar a sessão: $($_.Exception.Message)" "WARN"
            }
        }
    }
