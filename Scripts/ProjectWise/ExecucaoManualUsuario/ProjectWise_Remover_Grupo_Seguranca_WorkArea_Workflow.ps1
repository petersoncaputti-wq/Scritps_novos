<#
.SYNOPSIS
    Remove grupo da seguranca de workflows/states em Work Areas/folders do ProjectWise.

.DESCRIPTION
    Localiza concessao/projetos dentro da estrutura Engenharia e remove um Group
    da DocumentSecurity configurada por Workflow e State.

    Por seguranca, o script roda em simulacao por padrao. Use -Executar para
    gravar a remocao no ProjectWise.

.EXAMPLE
    powershell.exe -NoProfile -MTA -ExecutionPolicy Bypass -File ".\ProjectWise_Remover_Grupo_Seguranca_WorkArea_Workflow.ps1"

.EXAMPLE
    powershell.exe -NoProfile -MTA -ExecutionPolicy Bypass -File ".\ProjectWise_Remover_Grupo_Seguranca_WorkArea_Workflow.ps1" -DatasourceName "01SSRV305.ECSC.ECORODOVIAS.CORP:Ecorodovias-01" -NomeGrupo "NOME DO GROUP" -ConcessaoNome "Ecovias"

.EXAMPLE
    powershell.exe -NoProfile -MTA -ExecutionPolicy Bypass -File ".\ProjectWise_Remover_Grupo_Seguranca_WorkArea_Workflow.ps1" -NomeGrupo "NOME DO GROUP" -WorkflowNames "Workflow - Engenharia" -StateNames "Em elaboracao","Superado" -Executar
#>
...
param(
    ...
    [string[]]$WorkflowNames = @("Workflow - Engenharia", "Workflow - Engenharia - Unidade", "Workflow - Transferencia", "Workflow - Comentarios", "Importacao"),
    [string[]]$StateNames = @(),
    [switch]$Executar,
...
$ArquivoLog = Join-Path $PastaLogs "PW_RemoverGrupoSegurancaWorkAreaWorkflow_$TimeStamp.log"
$ArquivoCsv = Join-Path $PastaLogs "PW_RemoverGrupoSegurancaWorkAreaWorkflow_$TimeStamp.csv"
$script:LoginPW = $null
...
function Resolver-GrupoPorTexto {
    ...
    while ([string]::IsNullOrWhiteSpace($Texto)) {
        $Texto = (Read-Host "Nome do Group que sera removido dos workflows/states").Trim()
    }
...
function Obter-NomeState {
    ...
    return Obter-ValorSeguroPropriedade -Objeto $State -PossiveisNomes @("Name", "StateName", "WorkflowState", "ObjectName")
}

function Obter-StatesDoWorkflow {
    param([string]$WorkflowName)

    if ($StateNames -and $StateNames.Count -gt 0) {
        return @($StateNames | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
    }

    if (-not (Get-Command Get-PWWorkflowStateLinks -ErrorAction SilentlyContinue)) {
        throw "Cmdlet Get-PWWorkflowStateLinks nao encontrado. Informe -StateNames manualmente."
    }

    try {
        $states = @(Get-PWWorkflowStateLinks -WorkflowName $WorkflowName -ErrorAction Stop | Where-Object { $null -ne $_ })
        $nomes = @($states | ForEach-Object { Obter-NomeState -State $_ } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) } | Sort-Object -Unique)

        if ($nomes.Count -eq 0) {
            Write-Warn "Nenhum state retornado para o workflow '$WorkflowName'."
        }
        else {
            Write-Log "Workflow '$WorkflowName': $($nomes.Count) state(s) encontrados."
        }

        return $nomes
    }
    catch {
        Write-Warn "Falha ao obter states do workflow '$WorkflowName': $($_.Exception.Message)"
        return @()
    }
}

function Criar-AlvosRemocaoWorkflowState {
    $alvos = @()
    ...
    foreach ($workflow in $WorkflowNames) {
        ...
        $statesWorkflow = @(Obter-StatesDoWorkflow -WorkflowName $workflow)
        foreach ($state in $statesWorkflow) {
...
function Remover-GrupoDoWorkflowState {
    param(
        [object]$Pasta,
        [string]$Grupo,
        [string]$WorkflowName,
    ...
    if (-not (Get-Command Remove-PWFolderSecurity -ErrorAction SilentlyContinue)) {
        throw "Cmdlet Remove-PWFolderSecurity nao encontrado; nao foi possivel remover o grupo."
    }
    ...
    if (-not $Executar) {
        Write-Warn "Modo simulacao ativo. O Group '$Grupo' seria removido de '$nomePasta' | Workflow='$WorkflowName' | State='$StateName'. Use -Executar para gravar."
        return [PSCustomObject]@{
            Tipo = $TipoPasta
            Concessao = $NomeConcessao
            Nome = $nomePasta
            ...
            State = $StateName
            Status = "SIMULADO"
            Detalhe = "DocumentSecurity por workflow/state seria removida"
        }
    }
    ...
    try {
        ...
        Write-Log "Group '$Grupo' removido de '$nomePasta' | Workflow='$WorkflowName' | State='$StateName'." "OK"
        ...
        return [PSCustomObject]@{
            ...
            State = $StateName
            Status = "SUCESSO"
            Detalhe = "DocumentSecurity por workflow/state removida"
        }
    }
    catch {
        Write-Log "Falha ao remover Group '$Grupo' de '$nomePasta' | Workflow='$WorkflowName' | State='$StateName': $($_.Exception.Message)" "ERROR"
        return [PSCustomObject]@{
            Tipo = $TipoPasta
            Concessao = $NomeConcessao
            Nome = $nomePasta
...
try {
    ...
    Write-Host ""
    Write-Host "Group validado para remocao:" -ForegroundColor Yellow
    ...
    $alvosWorkflowState = @(Criar-AlvosRemocaoWorkflowState)
    if ($alvosWorkflowState.Count -eq 0) {
        throw "Nenhum workflow/state foi localizado para remocao. Informe -WorkflowNames e -StateNames manualmente."
    }

    $pastasAlvo = @()
    if (-not [string]::IsNullOrWhiteSpace($ProjetoNome)) {
        $projetoSelecionado = Selecionar-ProjetoDaConcessao -Concessao $concessaoSelecionada
        $pastasAlvo += [PSCustomObject]@{
            Tipo = "Projeto"
            Objeto = $projetoSelecionado
        }
    }
    else {
        $pastasAlvo += [PSCustomObject]@{
    ...
    Write-Host ""
    Write-Host "Plano de remocao:" -ForegroundColor Cyan
    Write-Host "Pastas alvo      : $($pastasAlvo.Count)"
    Write-Host "Workflow/States  : $($alvosWorkflowState.Count)"
    Write-Host "Modo             : $(if ($Executar) { 'EXECUCAO' } else { 'SIMULACAO' })"
    Write-Log "Plano de remocao montado. Pastas=$($pastasAlvo.Count) | WorkflowStates=$($alvosWorkflowState.Count) | Executar=$($Executar.IsPresent)"

    $resultados = @()
    foreach ($pastaAlvo in $pastasAlvo) {
        foreach ($alvo in $alvosWorkflowState) {
            $resultados += Remover-GrupoDoWorkflowState `
                -Pasta $pastaAlvo.Objeto `
                -Grupo $NomeGrupo `
                -WorkflowName $alvo.Workflow `
                -StateName $alvo.State `
    ...
    $resultados | Group-Object Status | ForEach-Object {
