# Versao de teste: estrutura base por poder concedente, sem disciplinas, com aplicacao de descricao na pasta raiz do ProjectWise.
$acessosEngenharia = @(
    'ASSISTENTE ENG',
    'CONSULTA',
    'ESPECIALISTA',
...
# -------------------------------------------------------
# Esta lista cria apenas a estrutura base/fases/volumes.
# As pastas de disciplinas deixam de ser criadas na abertura do projeto.
# Elas devem ser criadas sob demanda pelo script de movimentacao.
...
function CriarEstruturaInicialPastasProjeto {
    ...
    # Pastas base do projeto. As subpastas especificas de ARTESP/ANTT sao criadas em CriarPastasArea.
...
function CriarPastasArea {
    ...
    switch ($poderConcedente) {
        'ARTESP' { $pastas = $pastasBaseARTESP }
        'ANTT'   { $pastas = $pastasBaseANTT }
        Default  {
            Write-Host "Poder concedente invalido para estrutura de pastas: $poderConcedente"
            return
        }
    }
