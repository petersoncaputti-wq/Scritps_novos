...
try {
    ...
    foreach ($documento in $documentos) {
        ...
        if ((ValidaSeModeloFederadoAutoral -documento $documento)) {
            ...
            [array]$docsNaPastaDisciplina = Get-PWDocumentsBySearch -FolderPath $pastaDestino -FileName $documento.FileName -JustThisFolder -GetVersionsToo
            $criarVersao = $docsNaPastaDisciplina.Count -gt 0

            # Se -CreateVersion estiver presente, o comando não retorna nada
            ...
            if ($criarVersao) {
                [array]$docsNaPastaDisciplinaAposCopia = Get-PWDocumentsBySearch -FolderPath $pastaDestino -FileName $documento.FileName -JustThisFolder -GetVersionsToo
                if ($docsNaPastaDisciplinaAposCopia.Count -le $docsNaPastaDisciplina.Count) {
